...
from models import Drone, SimulationMap, Zone
...
class Pathfinder:
    ...
    def _is_link_available(
        ...
        occupancy = (
            self.link_reservations.get((z1, z2, turn), 0)
        ...
        for i in range(len(path_sequence)):
            ...
            if i > 0:
                ...
                for conn in self.sim_map.connections:
                    is_connection_match = (
                        (conn.zone1 == prev_zone and conn.zone2 == zone)
                        or (conn.zone1 == zone and conn.zone2 == prev_zone)
                    )
    ...
    def find_path_for_drone(
        ...
        while queue:
            ...
            if turn > max_turn_limit:
                print(
                    "Invalid map: start zone and end zone must "
                    "be on the same row or same column with no blocked zone "
                    "between."
                )
                exit(1)
            ...
            if current_zone.is_start or self._is_zone_available(
                current_zone,
                turn + 1,
            ):
                heapq.heappush(
    ...
    def assign_paths(self) -> None:
        for drone in self.drones:
            ...
            if best_path:
                ...
                print(
                    f"Warning: Could not find a path for "
